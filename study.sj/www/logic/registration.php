		...
		<form method="POST" enctype="multipart/form-data">			
			...
			<div class="submitButtonReg">
				<input type="submit" value="Sing up..." name="submit"></div>
			</br>
		</form>
...
<?php
    include_once("logic/bd.php");
				...
				//create table for reg users
	$sqlUsers = "CREATE TABLE  IF NOT EXISTS `users2` (
    ...
    if (isset($_POST['submit'])){		
		...
		elseif (!preg_match("/^[a-zA-Z0-9_\.\-]+@([a-zA-Z0-9\-]+\.)+[a-zA-Z]{2,6}$/", $_POST['email'])) {
			...
			$query = ("SELECT id FROM users2 WHERE login='$login'");
			...
			if (mysql_num_rows($sql) > 0) {
				...
				$query2 = ("SELECT id FROM users2 WHERE email='$email'");
				$sql = mysql_query($query2) or die(mysql_error());
				if (mysql_num_rows($sql) > 0){
					echo '<font color="red"><img border="0" src="media/images/error.gif">	User with this e-mail already exist!</font>';
				}
				else{
					$query = "INSERT INTO users2 (login, password, firstName, lastName, street, building, flat, phone, email, image )
