<?php
	...
	require_once("logic/Payment.class.php");
			...
			//create table
	$sqlRates = "CREATE TABLE  IF NOT EXISTS `rates` ( 
	...
	$rowAddress = mysql_fetch_array(mysql_query("SELECT * FROM users2 WHERE login='$login'"));
	...
	if(isset($_POST['saveRates']))	{	
		...
		if(!is_numeric($lightRate) || ($lightRate<0))	{	
			...
			elseif(empty($lightRate))	{
				...
				elseif($rowRates['moreLight'] != $moreLightRate)	{
...
<?php
	if(isset($_POST['countPayment']))	{	
							...
							//create table of index last payment only for registered users
		if($login)	{				
			$sqlIndex = "CREATE TABLE  IF NOT EXISTS `indexPayment` (
						 `id` INT NOT NULL AUTO_INCREMENT ,
		...
		if(((isset($_POST['lightCHB'])) && (($_POST['lightNow']-$_POST['lightPre'])>0) && (!(empty($_POST['lightRate'])))) ||
...
HERE;
				...
				if(isset($_POST['waterKitchenCHB']))	{
				...
				if(isset($_POST['sewageKitchenCHB']))	{
