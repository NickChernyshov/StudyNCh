<?php
	...
	$row = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE login='$login'"));
?>
					<!-- profile item -->
		<div id="main">
			<h2>Profile:</h2>
	...
	if(isset($_POST['saveChanges']))	{	
		...
		if($row['email'] != $_POST['email'])	{
			if (!preg_match("/^[a-zA-Z0-9_\.\-]+@([a-zA-Z0-9\-]+\.)+[a-zA-Z]{2,6}$/", $_POST['email'])) {
				...
				$email1=$_POST['email'];
				mysql_query("UPDATE users SET email='$email1' WHERE login='$login'");
				$save+=1;
			}			
		}
		...
		if($row['firstName'] != $_POST['firstName'])	{
			...
			mysql_query("UPDATE users SET firstName='$firstName' WHERE login='$login'");
			$save+=1;
		}
		...
		if($row['lastName'] != $_POST['lastName'])	{
			...
			mysql_query("UPDATE users SET lastName='$lastName' WHERE login='$login'");
			$save+=1;
		}
		...
		if($row['street'] !=$_POST['street'])	{
			...
			$street = $_POST['street'];
			mysql_query("UPDATE users SET street='$street' WHERE login='$login'");
			$save+=1;
		}
		...
		if($row['building'] !=$_POST['building'])	{
			...
			mysql_query("UPDATE users SET building='$building' WHERE login='$login'");
			$save+=1;
		}
		...
		if($row['flat'] !=$_POST['flat'])	{
			...
			$flat = $_POST['flat'];
			mysql_query("UPDATE users SET flat='$flat' WHERE login='$login'");
			$save+=1;
		}
		...
		if($row['phone'] != $_POST['phone'])	{
			...
			if(!is_numeric($_POST['phone']))	{
				...
				mysql_query("UPDATE users SET phone='$phone' WHERE login='$login'");
				$save+=1;
			}
		}	
		...
		if($_FILES['filename']['name'] != "")	{
			...
			mysql_query("UPDATE users SET image='$image' WHERE login='$login'");
			}	else	{
